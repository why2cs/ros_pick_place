...
int main(int argc, char **argv){
    ...
    ros::NodeHandle nh;
	ROS_INFO("==========>START Camera Intrinsic Calibration, press keys on IMAGE WINDOW: [Enter]:add data [Esc]:exit<==========");
	...
	try{
		...
		while (!exitCollectData && ros::ok()) {
			...
			if (keyValue == 27) {									//【Esc】：将退出标定数据采集的标志设置为真，退出while循环
				...
				if (findGrid) {
                	...
                	ROS_INFO("[%d] sets of data have been added...",imagePointsVec.size());
		...
		if (imagePointsVec.size() < 4) {
			...
			intrinsicCalibrationFile << "cameraMatrix" << cameraMatrix << "distCoeffs" << distCoeffs;
			intrinsicCalibrationFile.release();

			ROS_INFO_STREAM("camera intrinsic calibration finished, and the result has been saved as: " << calibrationInfoUrl << endl
                    << "camera Matrix: " << cameraMatrix << endl
                    << "camera distCoeffs：  k1:" << distCoeffs[0] << "  k2:" << distCoeffs[1]
                    << "  p1:" << distCoeffs[2] << "  p2:" << distCoeffs[3] << "  k3:" << distCoeffs[4] << endl
	...
	ROS_INFO("==========>Camera Intrinsic Calibration STOPPED!<==========");
	return 0;
}